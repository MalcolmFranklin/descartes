...
name_sequence : cap_char L_PAR cap_char R_PAR  
		...
		strcpy(item_str,$1.pstr);
		strcat(item_str,"(");
		strcat(item_str,$3.pstr);
		strcat(item_str,")");
		$$.pstr=(char *)calloc(1+strlen(item_str),sizeof(char));
		strcpy($$.pstr,item_str);
	...
	| name_sequence cap_char L_PAR cap_char R_PAR  
		{printf(" rule 32\n");
		strcat(item_str,$2.pstr);
		strcat(item_str,"(");
		strcat(item_str,$4.pstr);
		strcat(item_str,")");
		}
...
para :  INDENT REFER nls 
		...
		node_type=REFERENCE_DIRECT_PRODUCT;
               Build_Hoare_Tree("%d %s %d",node_type,$2.pstr,1);
...
return :	 RETURN nls
		...
		node_type = MATCH_DIRECT_PRODUCT;
             Build_Hoare_Tree("%d %s %d",node_type,$1.pstr,1);

		}
	| RETURN operators nls
		{printf("rule 102 %s %s indent%d\n",$1.pstr,$2.pstr,indentation);
		strcpy(item_str,$1.pstr);
		strcat(item_str,$2.pstr);
		if(strcmp($2.pstr, "*") ==0){
...
node_set : Indent node_name nls 
		...
		node_type=REFERENCE_DIRECT_PRODUCT; 
		printf("rule 121 %s %d\n",item_str,node_type);
              Build_Hoare_Tree("%d %s %d",node_type,$2.pstr,tmp_indent);
		}
	| Indent error nls
	| Indent node_name operators  nls
		{printf("rule 122 %s %s %d indent%d\n",$2.pstr,$3.pstr,$2.val,tmp_indent);
		strcpy(item_str,$2.pstr);
		strcat(item_str,$3.pstr);
		printf("rule 122 %s %s %d indent%d\n",item_str,$3.pstr,item_val,tmp_indent);
	...
	|  node_set Indent node_name nls
		...
		printf("rule 123%s %d\n",item_str,node_type);
             Build_Hoare_Tree("%d %s %d",node_type,$3.pstr,tmp_indent);

		}
	|  node_set Indent error nls
	| node_set Indent  node_name operators nls
		{printf("rule 124 %s %s %d indent %d\n",$3.pstr,$4.pstr,$3.val,tmp_indent);
		strcpy(item_str,$3.pstr);
		strcat(item_str,$4.pstr);
		printf("rule 124 %s %s %d indent %d\n",item_str,$4.pstr,item_val,tmp_indent);
