/* @(#) mem.h 1.1@(#) Solbourne id 9/22/93 00:03:31 */
/*
 * Copyright 1988 Solbourne Computer, Inc.
 * All rights reserved.
 */

/*	@(#)mem.h 1.3 90/12/05 SMI	*/
...
#ifdef	solbourne
#define	M_DELAY		16	/* /dev/delay - delayed version of /dev/zero */
#endif	solbourne

#ifdef	solbourne
#define M_VME16D16_SUP	0x10+5	/* /dev/vme16d16 - VME 16bit addr/16bit data */
#define M_VME24D16_SUP	0x10+6	/* /dev/vme24d16 - VME 24bit addr/16bit data */
#define M_VME32D16_SUP	0x10+7	/* /dev/vme32d16 - VME 32bit addr/16bit data */
#define M_VME16D32_SUP	0x10+8	/* /dev/vme16d32 - VME 16bit addr/32bit data */
#define M_VME24D32_SUP	0x10+9	/* /dev/vme24d32 - VME 24bit addr/32bit data */
#define M_VME32D32_SUP	0x10+10	/* /dev/vme32d32 - VME 32bit addr/32bit data */

#define	M_DEV_MASK	0x1f	/* device mask for minor number */
#define	M_IOBUS_MASK	0xe0	/* iobus mask for minor number */
#define	M_IOBUS_SHIFT	5	/* iobus shift for minor number */
#endif	solbourne
...
#ifdef	solbourne
/* define for viking mmu breakpoint diags */
struct	mdiag {
	u_int	mdiag_bkv[2];
	u_int	mdiag_bkm[2];
	u_int	mdiag_bkc_pad;
	u_int	mdiag_bkc;		/* see mmu.h */
	u_int	mdiag_bks_pad;
	u_int	mdiag_bks;		/* see mmu.h */
	u_int	mdiag_action;		/* see mmu.h */
};
#define	MIOGMDIAG	_IOWR(M, 10, struct mdiag)
#define	MIOSMDIAG	_IOWR(M, 11, struct mdiag)

/* define for forcing ECC errors in memory */
struct	mecc {
	caddr_t	mecc_vaddr;		/* argument to ioctl */
	caddr_t mecc_paddr;		/* filled in on return */
	int	mecc_bit_err[2];	/* bits to corrupt */
};
#define	MIOCSVECCS	_IOWR(M, 12, struct mecc) /* virtual single bit */
#define	MIOCSVECCM	_IOWR(M, 13, struct mecc) /* virtual multi bit */
#define	MIOCSPECCS	_IOWR(M, 14, struct mecc) /* physical single bit */
#define	MIOCSPECCM	_IOWR(M, 15, struct mecc) /* physical multi bit */
#endif	solbourne
#endif /*!_sun_mem_h*/
