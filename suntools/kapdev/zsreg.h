/* @(#) zsreg.h 1.1@(#) Solbourne id 9/22/93 00:10:26 */
/*	@(#)zsreg.h 1.3 90/12/05 SMI	*/
...
#ifdef	solbourne
/*
 * The KBUS I/O board requires that the uart for the keyboard and
 * mouse be present, It also requires special programming because it
 * has been overloaded with additional duties (i.e. system clock).
 * We require that it be configured as zs1 or we will panic when probing.
 * We also do not allow reprogramming of the uart by the user.
 */
#define	ZS_TTY_CTLR	0	/* controller for tty ports */
#define	ZS_KM_CTLR	1	/* controller for kbd/mouse */
#define	ZS_KBD_UNIT	3	/* unit for keyboard port */
#define	ZS_MOUSE_UNIT	2	/* unit for mouse port */

/* Handy macros for determining port usage type */
#define	ZS_ISKBD(x)	((((x) & 017) >= ZS_KBD_UNIT) && (((x) & 1) == 1))
#define	ZS_ISMOUSE(x)	((((x) & 017) >= ZS_MOUSE_UNIT) && (((x) & 1) == 0))
#endif	solbourne
