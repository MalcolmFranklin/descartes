/* @(#) maskbits.h 1.1@(#) Solbourne id 9/22/93 00:06:52 */
/*
...
******************************************************************/
/* $Header: /i/int/os/4.1C/RCS/maskbits.h,v 1.1 1990/08/15 15:32:05 barach Exp $ */
...
#define maskpartialbits32(x, w, mask) \
    mask = startpartial32[(x) & 0x1f] & endpartial32[((x) + (w)) & 0x1f];

#define mask32bits(x, w, startmask, endmask) \
    startmask = starttab32[(x)&0x1f]; \
    endmask = endtab32[((x)+(w)) & 0x1f];
...
{ \
    int m; \
    m = 32-(x); \
    dst = (SCRLEFT(*(psrc), (x)) & endtab32[m]) | \
	  (SCRRIGHT(*((psrc)+1), m) & starttab32[m]); \
}
