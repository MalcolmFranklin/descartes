/* @(#) audio_79C30.h 1.1@(#) Solbourne id 9/22/93 00:05:06 */
/*
 * Copyright 1991 Solbourne Computer, Inc.
 * All rights reserved.
 */
/* @(#)audio_79C30.h 1.2 90/12/05 SMI */

/*
 * Copyright (c) 1989, Sun Microsystems Inc.
 */

#ifndef _sbusdev_audio_79C30_h
#define	_sbusdev_audio_79C30_h
...
/* Macros for detecting the control device */
#define	AUD_CTLFLAG		(0x80)
#define	AUD_CTLMASK		(AUD_CTLFLAG-1)
#define	AUD_ISCTL(dev)		((dev & AUD_CTLFLAG) != 0)
#define	AUD_UNIT(dev)		(dev & AUD_CTLMASK)
 ...
 */
#ifdef	solbourne
#define PAD(x) u_char _pad/**/x[7]
#else	solbourne
#define PAD(x)
#endif	solbourne
struct aud_79C30_chip {
    char	cr;		/* Command Register */		   PAD(0);
    char	dr;		/* Data Register */		   PAD(1);
    char	dsr1;		/* D-channel Status Register 1 */  PAD(2);
    char	der;		/* D-channel Error Register */	   PAD(3);
    char	dctb;		/* D-channel Transmit Buffer */	   PAD(4);
    char	bbtb;		/* Bb Transmit Buffer */	   PAD(5);
    char	bctb;		/* Bc Transmit Buffer */	   PAD(6);
...
typedef struct {
	...
	struct aud_ctrlr {
	    struct aud_cmd	*cmdptr;	/* cmd chain list head */
	    unsigned int	samples;	/* number samples converted */
	    char		active;		/* conversion in-progress */
	    char		error;		/* over/under-flow */
...
#define	DEVP(v)	((aud_dev_t *)(v)->devdata)
#define	CSR(v)	(DEVP(v)->chip)
