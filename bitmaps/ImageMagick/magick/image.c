...
#define Reduce(weight) \
  ...
  */
  ...
  /*
    Sharpen image.
  */
  sharpened_image=SharpenImage(reduced_image);
  if (sharpened_image != (Image *) NULL)
    {
      DestroyImage(reduced_image);
      reduced_image=sharpened_image;
    }
    ...
    *q=background_pixel;
    ...
    for (x=1; x < (image->columns-1); x++)
    {
      ...
      */
      ...
      s=s1;
      Sharpen(-2); Sharpen(28); Sharpen(-2);
      s=s2;
      Sharpen(-1); Sharpen(-2); Sharpen(-1);
      if (total_red < 0)
        q->red=0;
      else
        if (total_red > (MaxRGB << 4))
          q->red=MaxRGB;
        else
          q->red=(total_red+7) >> 4;
      if (total_green < 0)
        ...
        if (total_green > (MaxRGB << 4))
          q->green=MaxRGB;
        else
          q->green=(total_green+7) >> 4;
      if (total_blue < 0)
        q->blue=0;
      else
        if (total_blue > (MaxRGB << 4))
          q->blue=MaxRGB;
        else
          q->blue=(total_blue+7) >> 4;
      q->index=0;
      q->length=0;
      q++;
      s0++;
